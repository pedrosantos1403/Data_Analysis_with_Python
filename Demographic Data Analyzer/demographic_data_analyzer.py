...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round(((df_high_education['salary'] == '>50K').sum() / len(df_high_education)) * 100, 1)
    
    df_low_education = df.loc[~df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])]
    lower_education_rich = round(((df_low_education['salary'] == '>50K').sum() / len(df_low_education)) * 100, 1)
    ...
    rich_percentage = round(((df_min_hours['salary'] == '>50K').sum() / len(df_min_hours)) * 100, 0)
    ...
    top_IN_occupation = df2['occupation'].value_counts().idxmax()
